...
function GlobeGroup({ position = [0, 0, 0], setMarker }: { position?: [number, number, number], setMarker: (v: THREE.Vector3) => void }) {
  ...
  useEffect(() => {
    ...
    const handlePointerUp = () => {
      isDragging.current = false;
      // Nach Loslassen: Weniger Schwung geben
      inertia.current.x *= 1.3;
      inertia.current.y *= 1.3;
    };
  ...
  useFrame(({ clock }) => {
    if (globeRef.current) {
      ...
      if (isDragging.current) {
        ...
        // Dämpfung (etwas schneller auslaufen)
        inertia.current.x *= 0.97;
        inertia.current.y *= 0.97;
        return;
      } else {
        if (isMobile) {
